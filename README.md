...
`POST /api/image-sets/generate` 使用 `multipart/form-data`，适合网站、自动化工具或第三方系统直接调用。
...
| `aspect_ratio` | 否 | 各模板默认值 | `1024x1024`、`1024x1536` 或 `1536x1024` |
| `auto_prompts` | 否 | `true` | 是否让 LLM 为每种套图扩写独立提示词 |
| `label` | 否 | `API 套图` | 返回任务的商品标签 |

```bash
curl -X POST http://localhost:8000/api/image-sets/generate \
...
```bash
curl -X POST http://localhost:8000/api/image-sets/result \
